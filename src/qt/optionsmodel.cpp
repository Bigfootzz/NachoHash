...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NAHA);
    ...
    if (!settings.contains("nAnonymizeNachoHashAmount"))
        settings.setValue("nAnonymizeNachoHashAmount", 1000);

    nAnonymizeNachoHashAmount = settings.value("nAnonymizeNachoHashAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNachoHashAmount"))
        SoftSetArg("-anonymizeNachoHashamount", settings.value("nAnonymizeNachoHashAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in NachoHash.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNachoHashAmount:
            return QVariant(nAnonymizeNachoHashAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNachoHashAmount:
            nAnonymizeNachoHashAmount = value.toInt();
            settings.setValue("nAnonymizeNachoHashAmount", nAnonymizeNachoHashAmount);
            emit anonymizeNachoHashAmountChanged(nAnonymizeNachoHashAmount);
            break;
        case CoinControlFeatures:
